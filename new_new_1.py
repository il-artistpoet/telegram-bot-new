# Работающий бот для Telegram с поддержкой Render
# Версия для деплоя на Render
...
def handle_tariff_selection(call):
    ...
    try:
        ...
        # Проверяем, если пользователь уже в базе и оплатил
        if user_data is not None and len(user_data) == 2:
            current_tariff, paid = user_data
            
            logger.info(f"🔍 Найден пользователь: тариф='{current_tariff}', оплата={paid}")
            
            # Если уже оплатил - предлагаем апгрейд
            if paid == 1:  # Явно проверяем на 1
                if current_tariff == "читатель" and tariff == "участник":
                    ...
                    bot.answer_callback_query(call.id, f"У вас уже есть тариф {current_tariff}")
                    return
                else:
                    ...
                    bot.send_message(
                        user_id,
                        f"✅ Вы уже оплатили тариф '{current_tariff.upper()}'!\n\n"
                        f"Если хотите изменить тариф, напишите @artistilja",
                        parse_mode=None
                    )
                    return
        
        # Создаем/обновляем запись для пользователя (ИСПРАВЛЕННАЯ СТРОКА)
        cursor.execute("""
            INSERT INTO users (user_id, tariff, amount, clicked_link, updated_at) 
        ...
        logger.info(f"✅ Запись сохранена, проверка paid={check[0] if check else 'нет'}")
...
# ========== ПРОВЕРКА ПОДПИСОК (РУЧНОЙ РЕЖИМ) ==========
@bot.message_handler(commands=['check_subs'])
def check_all_subscriptions(message):
    """Ручная проверка всех подписок (только для админа)"""
    if message.from_user.id != ADMIN_ID:
        bot.reply_to(message, "❌ Эта команда только для администратора")
        return
    
    logger.info(f"🔍 Админ {ADMIN_ID} запустил проверку подписок")
    ...
    try:
        ...
        cursor.execute("""
            ...
            ORDER BY screenshot_date
        """)
        
        all_users = cursor.fetchall()
        
        if not all_users:
            bot.reply_to(message, "📭 Нет пользователей с оплаченными подписками")
            return
...
def subscription_status(message):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM users WHERE paid = 1 AND screenshot_date IS NOT NULL")
        total_paid = cursor.fetchone()[0] or 0
        
        # Активные (меньше 30 дней)
        cursor.execute("""
            SELECT COUNT(*) FROM users 
            WHERE paid = 1 
            AND screenshot_date IS NOT NULL
        ...
        # Почти истекшие (27-29 дней)
        cursor.execute("""
            SELECT COUNT(*) FROM users 
            WHERE paid = 1 
            AND screenshot_date IS NOT NULL
            AND julianday('now') - julianday(screenshot_date) BETWEEN 27 AND 29
        """)
        expiring_soon = cursor.fetchone()[0] or 0
        
        # Истекшие (больше 30 дней, но paid ещё 1)
        cursor.execute("""
            SELECT COUNT(*) FROM users 
            WHERE paid = 1 
            AND screenshot_date IS NOT NULL
...
• Скоро истекают (3 дня): {expiring_soon}
• Просрочены (требуют проверки): {expired_but_not_updated}

🔍 Для проверки: /check_subs
👥 Для списка: /list
        """
...
if __name__ == "__main__":
    ...
    # Определяем, запускаем ли на Render
    ...
    if is_render:
        ...
        if render_url:
            ...
            logger.warning("⚠️ RENDER_EXTERNAL_URL не найден, вебхук не настроен")
        
        # Запускаем Flask сервер
        logger.info("🚀 Запускаем Flask сервер на порту 8080...")
        app.run(host='0.0.0.0', port=8080, debug=False, use_reloader=False)
        
    else:
        logger.info("📱 Локальный запуск (для тестирования)")
        ...
        try:
            ...
            logger.info("✅ Бот готов к работе с polling...")
            
            # Запускаем polling для локального тестирования
