...
def debug_command(message):
    """Проверка, обновился ли код на Render"""
    import os
    import datetime
    ...
    # Проверяем, какой текст в коде
    if "Комментарии без ответа автора" in content:
        tariff_status = "✅ НОВЫЙ текст в коде (обновился)"
    else:
        tariff_status = "❌ СТАРЫЙ текст в коде (нужен redeploy)"
    
    # Проверяем другие метки
    checks = []
    if "Комментарии без ответа автора" in content:
        checks.append("✓ 'Комментарии без ответа автора'")
    if "без обратной связи" in content.lower():
        checks.append("✗ 'без обратной связи' (старый)")
    
    # Отправляем отчет
    response = (
        "🔍 *ОТЛАДОЧНАЯ ИНФОРМАЦИЯ*\n\n"
        f"📁 Файл: `{os.path.basename(current_file)}`\n"
        f"🕒 Изменен: `{mod_time_str}`\n"
    ...
    # Дополнительно: проверяем время запуска бота
    ...
    bot.send_message(
        message.chat.id,
        f"⏱ Бот запущен: {start_time.strftime('%d.%m.%Y %H:%M:%S')}\n"
        f"🔄 Аптайм: {uptime}"
    )

# Добавь эту переменную в начало кода (после импортов):
# start_time = datetime.datetime.now()
